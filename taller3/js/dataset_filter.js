...
function getTextByName(element, name){name = name.toUpperCase();
 for(var i=0; i<element.childNodes.length; i++)
 if (element.childNodes[i].nodeName.toUpperCase()==name)
...
function showlist(booklist){
 ...
 var count = (booklist.length > 22) ? 22 : booklist.length;
 ...
 removeChildren( dom_tracklist );
 dom_tracklist.appendChild( document.createTextNode("Mostrando " + count + " de " +
booklist.length));
}
...
function createLinkElement(url,text){
 ...
 } catch (e) {element = document.createElement( "a" );
